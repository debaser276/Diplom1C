...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПроцентОтРабот = ПроцентОтРабот(Специалист, Дата);
	Если Не ЗначениеЗаполнено(ПроцентОтРабот) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "У специалиста не установлен процент от работ";
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	
	СуммаЧасовКОплате = ВыполненныеРаботы.Итог("ЧасыКОплатеКлиенту");
	СтавкаЧаса = СтоимостьЧасаРаботыПоДоговору(Договор);			
	Если СуммаЧасовКОплате > 0 Тогда
		...
		Движение.СуммаКОплате = СуммаЧасовКОплате * СтавкаЧаса;
		
		Движение = Движения.ВКМ_ВыполненныеСотрудникомРаботы.Добавить();
		Движение.Период = Дата;
		Движение.Сотрудник = Специалист;
		Движение.ЧасовКОплате = СуммаЧасовКОплате;
		Движение.СуммаКОплате = СуммаЧасовКОплате * СтавкаЧаса * ПроцентОтРабот / 100;
...
Функция ПроцентОтРабот(Сотрудник, Дата)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ВКМ_УсловияОплатыТрудыСрезПоследних.ПроцентОтРабот КАК ПроцентОтРабот
		|ИЗ
		|	РегистрСведений.ВКМ_УсловияОплатыТруды.СрезПоследних(&Дата, Сотрудник = &Сотрудник) КАК
		|		ВКМ_УсловияОплатыТрудыСрезПоследних";
